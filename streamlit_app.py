...
def extract_image_text(file_path: Path) -> str:
    ...
    if not model:
        raise RuntimeError("OCR indisponible sans GEMINI_API_KEY.")
    prompt = (
...
def answer_with_gemini(question: str, chunks: List[Chunk]) -> str:
    model = get_gemini_model()
    if not model:
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("Effacer cette session", use_container_width=True, key="clear_session_button"):
            reset_session()
    ...
    if st.button("Obtenir la reponse", use_container_width=True, key="answer_button"):
